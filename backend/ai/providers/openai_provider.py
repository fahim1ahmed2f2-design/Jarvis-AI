...
logger = logging.getLogger("jarvis.provider.openai")
...
class OpenAIProvider(BaseLLMProvider):
    ...
    def generate_chat(self, messages: List[Dict[str, str]], system_prompt: str, temperature: float = 0.7) -> str:
        ...
        with httpx.Client(timeout=30.0) as client:
            ...
            if res.status_code != 200:
                ...
                raise RuntimeError(err_data.get("message", f"OpenAI API Error {res.status_code}"))
            data = res.json()
    ...
    def test_connection(self) -> Dict[str, Any]:
        try:
            ...
            return {"success": True, "message": f"Connected successfully to OpenAI ({self.model})", "model_id": self.model}
